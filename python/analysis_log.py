...
if __name__=='__main__':
    ...
    df=read_log('0608',ServoController(0x10))
    
    JST = timezone(timedelta(hours=+9))
    start=datetime(2024,6,8,4,0,0,tzinfo=JST)-timedelta(seconds=0)
    end=  start + timedelta(seconds=60*60*5)
    ...
    df=pd.merge(df,read_log('0608',Tachometer(0x21)),how='outer',on=['utc','jst'])
    df=pd.merge(df,read_log('0608',Pitot(0x31)),how='outer',on=['utc','jst'])
